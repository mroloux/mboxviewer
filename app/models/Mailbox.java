			...
			for (int i = 0; i < inputAsBytes.length - 5; ++i) {
				if (messageMarkerAt(inputAsBytes, i)) {
					if (previousMessageMarkerFound) {
						emails.add(new Message(new ByteArrayInputStream(inputAsBytes, startIndexOfMessage, i - startIndexOfMessage)));
					}
					previousMessageMarkerFound = true;
					startIndexOfMessage = i;
				}
			}
			emails.add(new Message(new ByteArrayInputStream(inputAsBytes, startIndexOfMessage, inputAsBytes.length - startIndexOfMessage)));
